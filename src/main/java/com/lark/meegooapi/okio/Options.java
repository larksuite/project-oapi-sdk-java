...
public final class Options extends AbstractList<ByteString> implements RandomAccess {
     ...
     * <p>
     * SELECT nodes are encoded as: - selectChoiceCount: the number of bytes to choose between (a
     * positive int) - prefixIndex: the result index at the current position or -1 if the current
     * position is not a result on its own - a sorted list of selectChoiceCount bytes to match against
     * the input string - a heterogeneous list of selectChoiceCount result indexes (>= 0) or offsets
     * (< 0) of the next node to follow. Elements in this list correspond to elements in the preceding
     * list. Offsets are negative and must be multiplied by -1 before being used.
     * <p>
     * SCAN nodes are encoded as: - scanByteCount: the number of bytes to match in sequence. This
     * count is negative and must be multiplied by -1 before being used. - prefixIndex: the result
     * index at the current position or -1 if the current position is not a result on its own - a list
     * of scanByteCount bytes to match - nextStep: the result index (>= 0) or offset (< 0) of the next
     * node to follow. Offsets are negative and must be multiplied by -1 before being used.
     * <p>
