 ...
 * {@link BufferedSource} which is both more efficient and more convenient. Use {@link
 ...
 * heterogeneous: a {@code DataInputStream} for primitive values, a {@code BufferedInputStream} for
 * buffering, and {@code InputStreamReader} for strings. This class uses {@code BufferedSource} for
 * all of the above.
 *
 * <p>Source avoids the impossible-to-implement {@linkplain
 * java.io.InputStream#available available()} method. Instead callers specify how many bytes they
 * {@link BufferedSource#require require}.
 *
 * <p>Source omits the unsafe-to-compose {@linkplain java.io.InputStream#mark
 * mark and reset} state that's tracked by {@code InputStream}; instead, callers just buffer what
 * they need.
 *
 * <p>When implementing a source, you don't need to worry about the {@linkplain
 * java.io.InputStream#read single-byte read} method that is awkward to implement efficiently and
 * returns one of 257 possible values.
 *
 ...
 * Use {@link Okio#source} to adapt an {@code InputStream} to a source. Use {@link
 * BufferedSource#inputStream} to adapt a source to an {@code InputStream}.
 */
public interface Source extends Closeable {

    /**
     * Removes at least 1, and up to {@code byteCount} bytes from this and appends them to {@code
