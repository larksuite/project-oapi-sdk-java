 ...
 *
 */
package com.lark.meegooapi.okio;
...
import static com.lark.meegooapi.okio.Util.checkOffsetAndCount;
import static com.lark.meegooapi.okio.Util.reverseBytesLong;

/**
 * A collection of bytes in memory.
 *
 * <p><strong>Moving data from one buffer to another is fast.</strong> Instead
 * of copying bytes from one place in memory to another, this class just changes ownership of the
 ...
 * byte array in Java, the runtime must zero-fill the requested array before returning it to you.
 * Even if you're going to write over that space anyway. This class avoids zero-fill and GC churn by
 * pooling byte arrays.
 */
public final class Buffer implements BufferedSource, BufferedSink, Cloneable, ByteChannel {
     ...
     *                        example, this will return -2 if the buffer contains [ab] and the options
     *                        are [abc, abd]. Note that this is made complicated by the fact that
     *                        options are listed in preference order, and one option may be a prefix
     *                        of another. For example, this returns -2 if the buffer contains [ab] and
    ...
    /**
     * Returns a tail segment that we can write at least {@code minimumCapacity} bytes to, creating it
     * if necessary.
     */
    ...
    /**
     * Returns the index of {@code b} in this at or beyond {@code fromIndex}, or -1 if this buffer
     * does not contain {@code b} in that range.
     */
    @Override
     ...
     * string like {@code [text=Hello]} or {@code [hex=0000ffff]}.
     */
    @Override
    public String toString() {
    ...
    public static final class UnsafeCursor implements Closeable {
        ...
        /**
         * Change the size of the buffer so that it equals {@code newSize} by either adding new capacity
