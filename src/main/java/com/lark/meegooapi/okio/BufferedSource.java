...
public interface BufferedSource extends Source, ReadableByteChannel {
    ...
    /**
     * Returns when the buffer contains at least {@code byteCount} bytes. Throws an {@link
     ...
     * allowed match. Use this to protect against streams that may not include {@code "\n"} or {@code
     * "\r\n"}.
     *
     * <p>The returned string will have at most {@code limit} UTF-8 bytes, and the maximum number
     * of bytes scanned is {@code limit + 2}. If {@code limit == 0} this will always throw an {@code
     * EOFException} because no bytes will be scanned.
     *
    ...
    /**
     * Returns the index of {@code b} if it is found in the range of {@code fromIndex} inclusive to
     * {@code toIndex} exclusive. If {@code b} isn't found, or if {@code fromIndex == toIndex}, then
     * -1 is returned.
     *
    ...
    /**
     * Returns true if {@code byteCount} bytes at {@code offset} in this source equal {@code bytes} at
     * {@code bytesOffset}. This expands the buffer as necessary until a byte does not match, all
